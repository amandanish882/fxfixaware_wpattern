...
| **Basis W-Shape** | Intraday calendar-spread basis pattern: `log(F_back/F_front)` from OHLCV-1m for .c.0/.c.1 across 435 sessions (Dec 2024 -- Apr 2026), stratified by calm / month-end / quarter-end / year-end |
...
| **Databento Loader** | MBP-1 (65d top of book), MBP-10 (10d deep book), SR3 OHLCV-1d (252d), FX back-month OHLCV-1d + OHLCV-1m (440d, .c.0 + .c.1 for basis) |
...
Starts KDB+ Personal Edition on port 5001, creates the `tick_data`, `rates`, `rfqs`, and `fills` tables, then loads CME MBP-10 deep book data via Databento for the four G4 contracts (cached as parquet) and seeds the tick store. The MBP-1 panel covers **65 trading days** of top-of-book aggregated to 15-minute returns; the MBP-10 panel covers **10 days** of deep book (10 levels) sliced to the 13:00-17:00 UTC London/NY overlap. A separate **OHLCV-1m** panel (1.38M rows, 440 calendar dates, Dec 2024 -- Apr 2026) provides full-day 1-minute bars for both `.c.0` (front) and `.c.1` (back-quarter) continuous contracts, used for the intraday calendar-spread basis analysis.
...
#### Basis W-Shape (Calendar-Spread Funding Pressure)

The spot W-shape is driven by the common USD-index move. To isolate the **funding-pressure component**, we compute the intraday calendar spread: `log(F_back / F_front) * 1e4` using CME OHLCV-1m data for both `.c.0` (front) and `.c.1` (back-quarter) continuous contracts. Spot cancels (both legs track the same underlying), leaving a pure forward-forward funding rate signal. The dataset covers **435 sessions** (Dec 2024 -- Apr 2026), stratified by day-type:

| Day Type | Sessions | London-Fix Basis Signal |
|---|---:|---|
| Calm | 399 | Near zero (< 1 bps) across all fix windows |
| Month-end | 30 | 3--6 bps pre-fix widening, sharp V-reversal at fix |
| Quarter-end | 4 | 10--18 bps reversal at fix (AUD -17.5, EUR -8.1 pre-fix) |
| Year-end | 2 | GBP -9.6 bps at-fix, JPY -7.4 pre-fix with sharp reversal |

<div align="center">
<img src="output/w_shape_basis_g4.png" alt="Intraday basis W-shape across G4 pairs" width="700"/>

*Intraday calendar-spread basis pattern around G4 daily fixes. Top: cumulative basis change through the full trading day (Tokyo, ECB, and London fixes marked). Bottom: mean basis change in fix windows around London 16:00 UTC, split by calm / month-end / quarter-end / year-end. The spot W-shape vanishes in calm-day basis; stress-day V-shaped reversals emerge at 5--18 bps magnitude.*
</div>
...
$$\mathbb{E}\bigl[\mathrm{PnL}\,\big|\,s\bigr] = P(\mathrm{hit} \mid s, \mathbf{x}) \cdot \bigl(s - c + \alpha_{\mathrm{per\text{-}trade}}\bigr) - \lambda \delta^2 + R(t)$$
